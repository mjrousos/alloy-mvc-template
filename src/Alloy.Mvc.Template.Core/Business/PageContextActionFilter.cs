...
namespace AlloyTemplates.Business
{
    ...
    public class PageContextActionFilter : IResultFilter
    {
        ...
        {
            var viewModel = (filterContext.Controller as Controller)?.ViewData.Model;

            var model = viewModel as IPageViewModel<SitePageData>;
            if (model != null)
            {
                var currentContentLink = filterContext.GetContentLink();

                var layoutModel = model.Layout ?? _contextFactory.CreateLayoutModel(currentContentLink, filterContext);
